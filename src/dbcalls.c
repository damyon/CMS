...
int loadVerifierCommentObjectID(int commentid, int *objid, void *sqlsock) {
  void *result = NULL;
  int errcode = 0;
  char *query = NULL, *objectid = NULL, *cidstr = NULL;
  
  *objid = 1;

  int2Str(commentid, &cidstr);

  vstrdupcat(&query, "select objectID from VerifierComments where commentID = ", cidstr, NULL);
  dhufree(cidstr);
...
int insertObjectDetails(ObjectDetails *details, int *newid, void *sqlsock) {
  ...
  nowstr = nowStr();
  vstrdupcat(&query, "insert into Objects (objectName, parentID, isOnline, type, isPublic, isDeleted, mimeType, version, lockedByUserID, publisherUserID, template, relativeOrder) values (",
                                             "'", safename, "', ", 
                                             details->parentID, ", ",
                                             "'", details->isOnline, "', ",
                                             "'", details->type, "', ",
...
int getVerifierComment(int commentid, VerifierComment **details, void *sqlsock) {
  ...
  char *query = NULL, *commentstr = NULL, *tmp = NULL;
  
  int2Str(commentid, &commentstr);
#ifdef PGSQL_DATABASE
  vstrdupcat(&query, "select commentID,objectID,userID,created,comment from VerifierComments where commentID = ", commentstr, NULL);
#else
  vstrdupcat(&query, "select commentID,objectID,userID,DATE_FORMAT(created, '%Y-%m-%d %H:%i:%s'),comment from VerifierComments where commentID = ", commentstr, NULL);
#endif
  dhufree(commentstr);
  ...
  comment = initVerifierComment();
  tmp = fetchSQLData(result, 0, 0);
  comment->commentID = strtol(tmp?tmp:"-1", NULL, 10);
  tmp = fetchSQLData(result, 0, 1);
  comment->objectID = strtol(tmp?tmp:"-1", NULL, 10);
  tmp = fetchSQLData(result, 0, 2);
  comment->userID = strtol(tmp?tmp:"-1", NULL, 10);
  tmp = fetchSQLData(result, 0, 3);
  comment->created = parseDateTime(tmp?tmp:"");
  tmp = fetchSQLData(result, 0, 4);
  comment->comment = dhustrdup(tmp);
...
int getObjectDetails(int objectid, ObjectDetails **details, void *sqlsock) {
  ...
  vstrdupcat(&query, "select objectID,objectName,parentID,isOnline,type,isPublic,mimeType,version,lockedByUserID,template,relativeOrder,publisherUserID from Objects where objectID = ", objectstr, NULL);
  dhufree(objectstr);
  ...
  tmp = fetchSQLData(result, 0, 11);
  obj->publisherUserID = dhustrdup(tmp?tmp:(char *)"");
  ...
  *details = obj;
  logDebug("getObjectDetails returned\n");
  return E_OK;
}
...
int copyVerifierComments(int oldid, int newid, void *sqlsock) {
  ...
  int2Str(oldid, &oldidstr);
  int2Str(newid, &newidstr);
  vstrdupcat(&query, "delete from VerifierComments where objectID = ", newidstr, NULL);
  errcode = runSQL(sqlsock, &result, query);
  dhufree(query);
  freeSQLResult(result); 

  vstrdupcat(&query, "create temporary table tmpV (userID integer, created timestamp, comment text)", NULL);
  ...
  vstrdupcat(&query, "insert into tmpV select userID, created, comment from VerifierComments where objectID = ", oldidstr, NULL);
  errcode = runSQL(sqlsock, &result, query);
  dhufree(query);
  if (errcode != 0) {
    dhufree(oldidstr);
  ...
  vstrdupcat(&query, "insert into VerifierComments (objectID, userID, created, comment) select ", newidstr, ", userID, created, comment from tmpV", NULL);
  dhufree(oldidstr);
  dhufree(newidstr);
  ...
  vstrdupcat(&query, "drop table tmpV", NULL);
  errcode = runSQL(sqlsock, &result, query);
  dhufree(query);
  if (errcode != 0) {
    logError("Query Error: %s\n", getSQLError(sqlsock));
...
int addVerifierCommentDB(VerifierComment *comment, void *sqlsock) {
  void *result = NULL;
  int errcode = 0;
  char *query = NULL, *idstr = NULL, *oidstr = NULL, *safecomment = NULL;

  int2Str(comment->userID, &idstr);
  int2Str(comment->objectID, &oidstr);
  safecomment = escapeSQLString(comment->comment);

  vstrdupcat(&query, "insert into VerifierComments (userID, objectID, comment) values (", idstr, ", ", oidstr, ", '", safecomment, "')", NULL);
  dhufree(idstr);
  dhufree(oidstr);
  dhufree(safecomment);
...
*******************************************************************************/
int setUserMetadata(int uid, char *name, char *value, void *sqlsock) {
  void *result = NULL;
  int errcode = 0;
  ...
  vstrdupcat(&query, "delete from UserMetadata where userID = ", idstr, " and fieldName = '", safename, "'", NULL);
  errcode = runSQL(sqlsock, &result, query);
  dhufree(query);

  freeSQLResult(result);
  vstrdupcat(&query, "insert into UserMetadata (userID, fieldName, fieldValue) values (", idstr, ", '", safename, "', '", safevalue, "')", NULL);
  dhufree(idstr);
  dhufree(safename);
  dhufree(safevalue);
...
int setObjectMetadata(int objid, char *name, char *value, void *sqlsock) {
  ...
  // indexObject(value, objid, sqlsock);
...
*******************************************************************************/
int getUserMetadata(int uid, char *name, char **value, void *sqlsock) {
  void *result = NULL;
  int errcode = 0;
  ...
  vstrdupcat(&query, "select fieldValue from UserMetadata where userID = ", idstr, " and fieldName = '", safename, "'", NULL);
  dhufree(idstr);
  dhufree(safename);
...
*******************************************************************************/
int getAllUserMetadata(int uid, char ***columns, int *numcols, void *sqlsock) {
  void *result = NULL;
  int errcode = 0, i = 0;
  ...
  vstrdupcat(&query, "select fieldName from UserMetadata where userID = ", idstr, " order by fieldName", NULL);
  dhufree(idstr);
...
*******************************************************************************/
int removeUserMetadata(int uid, char *name, void *sqlsock) {
  void *result = NULL;
  int errcode = 0;
...
int loadAllVerifierComments(int objectID, char ***columns, int *numcols, void *sqlsock) {
  void *result = NULL;
  int errcode = 0, i = 0;
  char *query = NULL, *oidstr = NULL, *tmp = NULL;

  int2Str(objectID, &oidstr);

  vstrdupcat(&query, "select commentID from VerifierComments where objectID = '", oidstr, "'", NULL);
  dhufree(oidstr);
...
int loadNotificationSettings(int objid, int *groupid, void *sqlsock) {
  ...
  vstrdupcat(&query, "select groupID from Notifications where objectID = ", objstr, NULL);
  dhufree(objstr);
...
int removeNotificationSettings(int objid, void *sqlsock) {
  ...
  int2Str(objid, &objstr);
  vstrdupcat(&query, "delete from Notifications where objectID = ", objstr, NULL);
  dhufree(objstr);
...
int saveNotificationSettings(int objid, int groupid, void *sqlsock) {
  ...
  vstrdupcat(&query, "insert into Notifications (objectID, groupID) values (", 
						objstr, ", ",
                                              	gidstr, ")", NULL);
  dhufree(objstr);
  dhufree(gidstr);
...
int dbCalCreateInstance(cal_instance *inst, void *sqlsock) {
  ...
  vstrdupcat(&query, "insert into cal_instance (objectPath) values ('", safepath, "');", NULL);
  dhufree(safepath);
...
int dbBoardCreateInstance(board_instance *inst, void *sqlsock) {
  ...
  safepath = escapeSQLString(inst->objectPath);
  vstrdupcat(&query, "insert into board_instance (objectPath) values ('", safepath, "');", NULL);
  dhufree(safepath);
