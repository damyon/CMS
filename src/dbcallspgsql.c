...
*******************************************************************************/
int runSQL(PGconn *conn, PGresult **result, char *query) {
  PGresult *res = NULL;
...
*******************************************************************************/
void freeSQLResult(PGresult *result) {
  if (result != NULL)
    PQclear(result);
}
...
*******************************************************************************/
int numSQLRows(PGresult *result) {
...
*******************************************************************************/
char *fetchSQLData(PGresult *result, int row, int field) {

  return (char *) PQgetvalue(result, row, field);
}
...
int deleteObjectDB(ObjectDetails *details, void *sqlsock) {
  ...
  char *query = NULL, *safename = NULL, *safemimetype = NULL, *safetemplate = NULL, *objidstr = NULL, *nowstr = NULL;
  ...
  safetemplate = escapeSQLString(details->template);
  nowstr = nowStr();
  vstrdupcat(&query, "insert into Objects (objectName,parentID,isOnline,type,isPublic,isDeleted,mimeType,version,lockedByUserID,template) values ('", safename, "', ", 
                               details->parentID, ", '", 
                               details->isOnline, "', '",
                               details->type, "', '",
                               details->isPublic, "', ",
  ...
  freeSQLResult(result);
  getSequenceValue("Objects_objectID_seq", &newid, connection);
...
int insertObjectDetails(ObjectDetails *details, int *newid, void *sqlsock) {
  ...
  nowstr = nowStr();
  vstrdupcat(&query, "insert into Objects (objectName, parentID, isOnline, type, isPublic, isDeleted, mimeType, version, lockedByUserID, template) values (",
                                             "'", safename, "', ", 
                                             details->parentID, ", ",
                                             "'", details->isOnline, "', ",
                                             "'", details->type, "', ",
...
int getObjectDetails(int objectid, ObjectDetails **details, void *sqlsock) {
  ...
  vstrdupcat(&query, "select objectID,objectName,parentID,isOnline,type,isPublic,mimeType,version,lockedByUserID,template from Objects where objectID = ", objectstr, NULL);
  dhufree(objectstr);
  ...
  obj->template = dhustrdup(tmp?tmp:"");
...
int getUserLoginDetails(char *username, char *password, int *userid, int *super, char **fullname, char **email, char **userType, int encrypt, void *sqlsock) {
  ...
  *email = dhustrdup(mail?mail:"");
  *userType = dhustrdup(type?type:"");
...
int loadFolderContentsDB(int objid, char *filter, int timestamp, char *sort, int **list, int *numobjs, void *sqlsock) {
  ...
  int i = 0, errcode = 0, id = 0, j = 0, k = 0, found = 0;
  ...
  vstrdupcat(&query, "select objectName from Objects where parentID = ", objidstr, " and version <= ", times, " and objectName like '", safefilter, "%' ", order, NULL);
  dhufree(objidstr);
  dhufree(safefilter);
  dhufree(times);
  dhufree(order);
...
int getWordID(char *w, int *id, void *sqlsock) {
  ...
  getSequenceValue("Dictionary_wordID_seq", id, connection);
  freeSQLResult(result);
  return E_OK;
}
...
int getUserDetails(int uid, UserDetails **details, void *sqlsock) {
  ...
  tmp = fetchSQLData(result, 0, 5);
  user->email = dhustrdup(tmp?tmp:"");
  tmp = fetchSQLData(result, 0, 6);
  user->userType = dhustrdup(tmp?tmp:"");
...
int updateObjectDB(int objectID, char *name, int ispublic, void *sqlsock) {
  ...
  PGresult *result = NULL;
  int errcode = 0;
  char *query = NULL, *oidstr = NULL, *safename = NULL, *nowstr = NULL;

  safename = escapeSQLString(name);
  int2Str(objectID, &oidstr);
  nowstr = nowStr();
  vstrdupcat(&query, "update Objects set isPublic = '", ispublic?"y":"n", "',objectName = '", safename, "', version = '", nowstr, "' where objectID = ", oidstr, NULL);
  dhufree(oidstr);
  dhufree(safename);
  dhufree(nowstr);
